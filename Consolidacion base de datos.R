...
ANT <- cod_mun_ant %>% 
  ...
  select(COD_MUNIC, MUNICIPIO = MUNICIPIO.y, tasa_nat, tasa_def, tasa_vio, tasa_bps, tot = pop.x)
...
ATL <- cod_mun_atl %>% 
  ...
  select(COD_MUNIC, MUNICIPIO, tasa_nat, tasa_def, tasa_vio, tasa_bps, tot = pop.x)

# Guardado ----
save(ATL, ANT, file = "Base_de_Datos.RData")
